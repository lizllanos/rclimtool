...
library2 <<- function(a.package){
  ...
  # Descarga de paquetes desde el CRAN --------------------------------------

if("ggplot2" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("ggplot2"))}
if("rtf" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("rtf"))}
if("gWidgets" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("gWidgets"))}
if("gWidgetsRGtk2" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("gWidgetsRGtk2")) }
if("RMAWGEN" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("RMAWGEN")) }
if("Kendall" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("Kendall")) }
if("tseries" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("tseries"))}
if("grid" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("grid"))}
if("car" %in% row.names(installed.packages())==FALSE){ suppressMessages(install.packages("car")) }
if("reshape" %in% row.names(installed.packages())==FALSE){ suppressMessages(install.packages("reshape"))}
if("maptools" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("maptools"))}
if("mapdata" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("mapdata"))}
if("RColorBrewer" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("RColorBrewer"))}
if("classInt" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("classInt"))}
if("DescTools" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("DescTools"))}
...
if("ncdf4" %in% row.names(installed.packages())==FALSE){suppressMessages(install.packages("ncdf4"))}

library2("devtools")
library2("gWidgetsRGtk2")  
#install_github("ecor/RMAWGEN") 
  ...
  # read.csv_n=function(x,...)read.csv(x,dec = Sys.localeconv()[1], sep= Sys.localeconv()[7],  ...)
  # write.csv_n=function(x,...)write.csv(x,dec = Sys.localeconv()[1], ...)
  

    if(Sys.localeconv()[1]==","){
      read.csv_n=function(x,...)read.csv2(x,check.names = F,...)
      write.csv_n=function(x,...)write.csv2(x,...)
    }

    if(Sys.localeconv()[1]=="."){
      read.csv_n=function(x,...)read.csv(x,check.names = F,...)
  ...
  load_tmin <- function(){
    tmin=read.csv(gfile("Seleccione el archivo de temperatura mínima"),dec = Sys.localeconv()[1],header=T,check.names = F)
    assign("tmin",tmin,.GlobalEnv)
  }
  
  load_tmax <- function(){
    tmax=read.csv(gfile("Seleccione el archivo de temperatura máxima"),dec = Sys.localeconv()[1],header=T,check.names = F)
    assign("tmax",tmax,.GlobalEnv)
  }
  
  load_precip <- function(){
    prec=read.csv(gfile("Seleccione el archivo de precitación"),dec = Sys.localeconv()[1],header=T,check.names = F)
    assign("prec",prec,.GlobalEnv)
  }
  
  load_coor <- function(){
    coor=read.csv(gfile("Seleccione el archivo de coordenadas"),dec = Sys.localeconv()[1],header=T,check.names = F)
    assign("coor",coor,.GlobalEnv)
  }
  ...
  graf_box <- function(){
    ...
    if(svalue(tipo)=="Mensual"){
      ...
      month1=dplyr::recode_factor(tmax$month,`1`='Ene',`2`='Feb',`3`='Mar',`4`='Abr',`5`='May',`6`='Jun',`7`='Jul',`8`='Ago',`9`='Sep',`10`='Oct',`11`='Nov',`12`='Dec',.ordered = T)
...
#      month1=factor(month1,levels=levels1)
  ...
  graf_disp <- function(){
    cat("\n\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n ")
    cat(paste0(" Generando gráficos dispersión "))
    cat("\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n ")
  ...
  graf_clim <- function(){
    ...
    cat("\n\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n ")
    cat(paste0(" Generando climogramas"))
    cat("\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n ")
    ...
    dir.create("Analisis gráfico",showWarnings=F)
    dir.create("Analisis gráfico/Climogramas",showWarnings=F)
    
    for(i in 1:(ncol(tmax_clim)-1)){
      jpeg(paste("Analisis gráfico/Climogramas/clim_",station[i],".jpeg",sep=""), width = 10, height = 7,units = 'in',res=200)
      par(mar = c(7,5,2.5,5))
      ...
      with(tmin_clim, plot(tmin_clim[,1],tmin_clim[,i+1], type="l", col="blue",lwd = 2,
                           axes=F, xlab=NA, ylab=NA, cex=1.2,ylim=c(min(tmin_clim[,i+1],na.rm=T)-0.5,max(tmax_clim[,i+1],na.rm=T)+0.5)))
      par(new = T)
  ...
  quality_control <- function(object){
    ...
    cat("\n\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n ")
    cat(" Iniciando control de calidad para ",svalue(variable))
    cat("\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n")
    
    object=eval(parse(text=svalue(variable)))
    ...
    if(svalue(variable)=="tmax"){
      dir.create("Control de calidad/tmax/Gráficos",showWarnings=F,recursive = T)
      ...
      {
        cat(paste0("Realizando control de calidad para la estación "),station[i],"\n")
        ...
        if(isTRUE(svalue(qc_na))) object[out_salt.n,i+3]<-NA
       ...
       if(isTRUE(svalue(qc_na))) object[c(out_atip),i+3]<-NA
        
        #out_atip_data = cbind(object[c(out_atip,out_atip.y),1:3],object[c(out_atip,out_atip.y),i+3])
        out_atip_data = cbind(object[c(out_atip),1:3],object[c(out_atip),i+3])
        ...
        tiff(paste0("Control de calidad/tmax/Gráficos/",station[i],"_",svalue(variable),"_box_qc.tiff"),compression = 'lzw',height = 10,width = 15,units="in", res=200)
        par(mfrow=c(2,1),
            oma = c(5,4,0,0) + 0.3)
        boxplot(object[,i+3]~month,range=as.numeric(svalue(criterio)),plot=T,xlab="",ylab="Temperatura máxima (°C)")
        mtext(paste0(station[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2)
        
        plot(1:length(object[,i+3]),object[,i+3],type="l",xaxt="n", xlab="",ylab="Temperatura máxima (°C)")
        ...
        tiff(paste0("Control de calidad/tmax/Gráficos/",station[i],"_",svalue(variable),"_line_graph_qc.tiff"),compression = 'lzw',height = 7,width = 16,units="in", res=150)
        ...
        points(c(out_atip),tmax[c(out_atip),i+3],col="black",bg="purple",cex=0.7,pch=21)
    ...
    if(svalue(variable)=="tmin"){
      dir.create("Control de calidad/tmin/Gráficos",showWarnings=F,recursive = T)
      ...
      for(i in 1:(ncol(object)-3))
      {
        ...
        tiff(paste0("Control de calidad/tmin/Gráficos/",station[i],"_",svalue(variable),"_box_qc.tiff"),compression = 'lzw',height = 10,width = 15,units="in", res=200)
        par(mfrow=c(2,1),
            oma = c(5,4,0,0) + 0.3)
        boxplot(object[,i+3]~month,range=as.numeric(svalue(criterio)),plot=T,xlab="",ylab="Temperatura mínima (°C)")
        mtext(paste0(station[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2)
        
        plot(1:length(object[,i+3]),object[,i+3],type="l",xaxt="n", xlab="",ylab="Temperatura mínima (°C)")
        lines(1:length(object[,i+3]),lim_inf,col="red",lty=2)
        lines(1:length(object[,i+3]),lim_sup,col="red",lty=2)
        axis(1,at=seq(1,nrow(tmin),nrow(tmin)/length(unique(object$year))),labels=seq(min(tmin$year),max(tmin$year),1),las=1,col="black",las =2,cex.axis = 0.8)
        
        dev.off()
        
        
        
        tiff(paste0("Control de calidad/tmin/Gráficos/",station[i],"_",svalue(variable),"_line_graph_qc.tiff"),compression = 'lzw',height = 7,width = 16,units="in", res=150)
        ...
        ylim1 = c(min(tmin[,i+3],na.rm=T) ,max(tmin[,i+3],na.rm=T)+1)
        ylim2 = c(min(object[,i+3],na.rm=T) ,max(object[,i+3],na.rm=T)+1)
        
        plot(tmin[,i+3],type="l",col="grey",xaxt="n",yaxt="n",xlab="",ylab="Temperatura mínima (°C)",main = "Antes del control de calidad",ylim=ylim1,cex.lab=0.8)
        ...
        points(c(out_atip),tmin[c(out_atip),i+3],col="black",bg="purple",cex=0.7,pch=21)
        ...
        plot(object[,i+3],type="l",col="grey",xaxt="n",yaxt="n",xlab="",ylab="Temperatura mínima (°C)",main = "Después del control de calidad",ylim=ylim2,cex.lab=0.8)
      ...
      porcentajes[i,]=cbind(round((dim(out_range_data)[1]/dim(tmin)[1])*100,2),round((dim(out_int_data)[1]/dim(tmin)[1])*100,2),round((dim(out_atip_data)[1]/dim(tmin)[1])*100),round((dim(out_salt_data)[1]/dim(tmin)[1])*100,2),round((dim(out_cons_data)[1]/dim(tmin)[1])*100,2),
                            round(na/dim(tmin)[1]*100,2))
    ...
    if(svalue(variable)=="prec"){
      ...
      dir.create("Control de calidad/prec/Gráficos",showWarnings=F,recursive = T)
     
      porcentajes=matrix(0,ncol(object)-3,4)
      ...
      for(i in 1:(ncol(object)-3))
      {
        ...
        val.na = boxplot(x~month,range=as.numeric(svalue(criterio)),plot=F)
        ...
        #out_atip.na_data = data.frame(c(out_atip.na),object[c(out_atip.na),i+3])
       
        
        out_atip_data = cbind(object[c(out_atip.na),1:3],object[c(out_atip.na),i+3])
        colnames(out_atip_data)<-c("day","month","year","value")
        ...
        if(isTRUE(svalue(qc_na))) object[c(out_atip.na),i+3]<-NA
        ##QC datos consecutivos 
        xc <-data.frame(hasta=cumsum(rle(object[,i+3])$lengths),cant_iguales=rle(object[,i+3])$lengths,valor=rle(object[,i+3])$values)
        xc$desde = xc$hasta -xc$cant_iguales + 1  
        xc <-na.omit(xc)
        ...
        porcentajes[i,]=cbind(round((dim(out_range_data)[1]/dim(prec)[1])*100,3),round((dim(out_atip_data)[1]/dim(prec)[1])*100),round((dim(out_cons_data)[1]/dim(prec)[1])*100,3),
                              round(na/dim(prec)[1]*100,3))
        
        #######gráficos control de calidad###########
        tiff(paste0("Control de calidad/prec/Gráficos/",station[i],"_",svalue(variable),"_box_qc.tiff"),compression = 'lzw',height = 10,width = 15,units="in", res=200)
        par(mfrow=c(2,1),
            oma = c(5,4,0,0) + 0.3)
        boxplot(x~month,range=as.numeric(svalue(criterio)),plot=T,xlab="",ylab="Precipitación (mm/día)")
        mtext(paste0(station[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2)
        
        plot(1:length(object[,i+3]),object[,i+3],type="l",xaxt="n", xlab="",ylab="Precipitación (mm/día)")
        lines(1:length(object[,i+3]),lim_inf.na,col="red",lty=2)
        lines(1:length(object[,i+3]),lim_sup.na,col="red",lty=2)
        axis(1,at=seq(1,nrow(prec),nrow(prec)/length(unique(object$year))),labels=seq(min(prec$year),max(prec$year),1),las=1,col="black",las =2,cex.axis = 0.8)
        
        dev.off()
        
        
        tiff(paste("Control de calidad/prec/Gráficos/",station[i],"_",svalue(variable),".line_graph_qc.tiff",sep=""),compression = 'lzw',height = 5,width = 16,units="in", res=150)
        ...
        ylim1 = c(min(prec[,i+3],na.rm=T) ,max(prec[,i+3],na.rm=T)+1)
        ylim2 = c(min(object[,i+3],na.rm=T) ,max(object[,i+3],na.rm=T)+1)
        
        plot(prec[,i+3],type="l",col="grey",xaxt="n",yaxt="n",xlab="",ylab="Precipitación (mm/día)",main = "Antes del control de calidad",ylim=ylim1,cex.lab=0.8)
        
        mtext(paste0(station[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2) 
        points(out_cons,prec[out_cons,i+3],col="black",bg="red",cex=0.7,pch=21)
        points(out_atip.na,prec[out_atip.na,i+3],col="black",bg="purple",cex=0.7,pch=21)
        
        axis(1,at=seq(1,nrow(prec),nrow(prec)/length(unique(object$year))),labels=seq(min(prec$year),max(prec$year),1),las=1,col="black",las =2,cex.axis = 0.8)
        axis(2,at=seq(ylim1[1],ylim1[2],20),labels=seq(ylim1[1],ylim1[2],20),las=1,col="black",las =1,cex.axis = 0.8)
        ...
        add_legend("bottomleft", legend=c("Datos Consecutivos","Datos Atípicos","Datos por fuera del rango"), pch=c(21,21,21), 
                   pt.bg= c("red","purple","green"),bty = "n",cex = 0.9,text.width=c(0.085,0.3,0.3),
                   horiz=TRUE)
  ...
  quality_control2 <- function(object){
    object=eval(parse(text=svalue(variable)))
    # val=ifelse(sapply(object[-3:-1],min2)>=as.numeric(svalue(minim)) & sapply(object[-3:-1],max2)<=as.numeric(svalue(maxim)),"está dentro del rango permitido","contiene valores por fuera del rango permitido")
    # val=cbind(val)
    # 
    ...
    if(svalue(variable)=="tmin"){
      ...
      for(i in 1:(ncol(object)-3))
      {
        ...
        # plot(object[,i+3],type="l",col="grey",xaxt="n",yaxt="n",xlab="",ylab="Temperatura mínima (°C)",main = "Después del control de calidad",ylim=ylim2,cex.lab=0.8)
        # 
        # axis(1,at=seq(1,nrow(tmin),366),labels=seq(min(tmin$year),max(tmin$year),1),las=1,col="black",las =2,cex.axis = 0.8)
        # axis(2,at=seq(ylim2[1],ylim2[2],5),labels=seq(ylim2[1],ylim2[2],5),las=1,col="black",las =1,cex.axis = 0.8)
        # 
        # add_legend("bottomleft", legend=c("Datos consecutivos","Datos atípicos","Datos por fuera del rango","Saltos consecutivos","Consistencia interna tmin<tmin"), pch=c(21,21,21,24,21), 
        #            pt.bg= c("red","purple","green","turquoise1","tan3"),bty = "n",cex = 0.9,text.width=c(0.085,0.3,0.235,0.3,0.3),
        #            horiz=TRUE)
        # 
        # dev.off()  
    ...
    if(svalue(variable)=="prec"){
      ...
      # dir.create("Control de calidad/prec/Gráficos",showWarnings=F)
      ...
      for(i in 1:(ncol(object)-3))
      {
        ...
        porcentajes[i,]=cbind(round((dim(out_range_data)[1]/dim(prec)[1])*100,2),round((dim(out_atip_data)[1]/dim(prec)[1])*100),round((dim(out_cons_data)[1]/dim(prec)[1])*100,2),
                              round(na/dim(prec)[1]*100,2))
        
        #######gráficos control de calidad###########
        # tiff(paste0("Control de calidad/prec/Gráficos/",station[i],"_",svalue(variable),"_box_qc.tiff"),compression = 'lzw',height = 10,width = 15,units="in", res=200)
        # par(mfrow=c(2,1),
        #     oma = c(5,4,0,0) + 0.3)
        # boxplot(object[,i+3]~month,range=as.numeric(svalue(criterio)),plot=T,xlab="",ylab="Precipitación (mm)")
        # mtext(paste0(station[i]),side=3,outer=T,adj=0,line=-1.3,cex = 1.2)
        # 
        # plot(1:length(object[,i+3]),object[,i+3],type="l",xaxt="n", xlab="",ylab="Precipitación (mm/día)")
        # lines(1:length(object[,i+3]),lim_inf.na,col="red",lty=2)
        ...
        # plot(prec[,i+3],type="l",col="grey",xaxt="n",xlab="",ylab="Precipitación (mm)",main=station[i])
        # points(out_cons,prec[out_cons,i+3],col="black",bg="red",cex=0.7,pch=21)
        # points(out_atip.na,prec[out_atip.na,i+3],col="black",bg="purple",cex=0.7,pch=21)
        # 
        # points(out_range,prec[out_range,i+3],col="black",bg="green",cex=0.7,pch=21)
        # axis(1,at=seq(1,nrow(prec),nrow(prec)/length(unique(object$year))*3),labels=seq(min(prec$year),max(prec$year),3),las=1,col="black")
        # 
        # 
        # 
        # add_legend("bottomleft", legend=c("Datos Consecutivos","Datos Atípicos","Datos por fuera del rango"), pch=c(21,21,21), 
  ...
  datos_falt <- function(){
    ...
    year_max <-as.numeric(max(tmax$year))
    year_min <- as.numeric(min(tmax$year))
    ...
    tmax<- read.csv_n("Control de calidad/tmax_qc.csv", header = TRUE)
    tmin<- read.csv_n("Control de calidad/tmin_qc.csv", header = TRUE)
    prec <- read.csv_n("Control de calidad/prec_qc.csv", header = TRUE)
    ...
    ifelse(file.exists("tmax_faltantes")=="FALSE",dir.create("Datos_faltantes/RMAWGEN/tmax_faltantes",showWarnings=F),"Ya existe carpeta")
    ifelse(file.exists("tmin_faltantes")=="FALSE",dir.create("Datos_faltantes/RMAWGEN/tmin_faltantes",showWarnings=F),"Ya existe carpeta")
    ifelse(file.exists("precip_faltantes")=="FALSE",dir.create("Datos_faltantes/RMAWGEN/prec_faltantes",showWarnings=F),"Ya existe carpeta")
    ...
    {
      jpeg(paste("Datos_faltantes/RMAWGEN/tmax_faltantes/tmax_",station[i],".jpeg",sep=""), width = 10, height = 7,units = 'in',res=200)
      ...
      jpeg(paste("Datos_faltantes/RMAWGEN/tmin_faltantes/tmin_",station[i],".jpeg",sep=""), width = 10, height = 7,units = 'in',res=200)
  ...
  datos_falt_chirps <- function(){
    cat("\n\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n ")
    cat(" Iniciando llenado de datos con CHIRPS ")
    cat("\n =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= \n")
    
    dir.create("Datos_faltantes/CHIRPS", showWarnings = F, recursive = T)
    
   
    prec <- read.csv_n("Control de calidad/prec_qc.csv", header = TRUE)
    prec=subset(prec,prec$year>=1980)
    yearmax <- max(prec$year)
    yearmin <- min(prec$year)
    
       
    station_data <- aggregate(prec[-3:-1],list(month=prec$month,year=prec$year),sum22)
    station_coord <- coor
    ...
    cat("Descargando datos de CHIRPS ...\n")
    
   download.file(paste0("http://iridl.ldeo.columbia.edu/SOURCES/.UCSB/.CHIRPS/.v2p0/.monthly/.global/.precipitation/Y/%28",ymin,"%29%28",ymax,"%29RANGEEDGES/X/%28",xmin,"%29%28",xmax,
              "%29RANGEEDGES/T/%28Jan%201981%29%28Dec%20",yearmax,"%29RANGEEDGES/data.nc"),destfile = "Datos_faltantes/CHIRPS/chirps.nc" ,mode="wb")
   
    
   chirps_all = stack("Datos_faltantes/CHIRPS/chirps.nc")
   
   st_cod = as.numeric(unlist(strsplit(names(station_data),"_")))
   st_cod = st_cod[!is.na(st_cod)]
   station_coord = station_coord[which(coor$code %in% st_cod),]
   
   station_chirps.b = raster::extract(x=chirps_all, y=station_coord[,c("lon","lat")], method = 'bilinear')
   station_chirps.b = as.data.frame(t(station_chirps.b))[1:nrow(station_data),]
   names(station_chirps.b)=names(station_data)[-2:-1]
   
   dates=seq(as.Date(paste0(yearmin,"/01/01")),as.Date(paste0(yearmax,"/12/31")),"month")
   ...
   for (i in 1:ncol(station_chirps.b)){
     cat(paste0("Generando llenado de datos para la estación "),names_st[i],"\n")
     ...
     tiff(paste0("Datos_faltantes/CHIRPS/prec_",names_st[i],"_monthly.tiff"),compression = 'lzw',height = 10,width = 10,units="in", res=200)
     par(mfrow=c(2,1))
     
     plot(dates,station_data[,i+2],lwd=1.5,type="l",xlab="",ylab="Precipitation (mm/mes)",main=names_st[i])
     lines(dates,station_chirps.b[,i],col="red",lty=2,lwd=1)
     lines(dates,data_model,col="blue",lty=2)
     
     
     plot(station_chirps.b[,i],station_data[,i+2], ylab="Observed_stations",xlab="CHIRPS")
   ...
   write.csv(station_data,paste0("Datos_faltantes/CHIRPS/monthly_fill_prec.csv"),row.names = F)
  ...
  inf <- function(h,...){ #Crea y genera informe en word
    ...
    addHeader(info,title="Aplicativo para análisis de series de clima",subtitle=paste("Informe para la variable",svalue(nom_val2)),font.size=14)
  ...
  inf2 <- function(h,...){ #Crea y genera informe en word
    confirmDialog(paste("El informe se guardará en",getwd()),"Ubicación archivos")
    
    info<-RTF(paste(svalue(nom_arch1),"_",svalue(variable),".doc"),width=8.5,height=11,font.size=10,omi=c(1,1,1,1))
    
    addHeader(info,title="Aplicativo para análisis de series de clima",subtitle="Control de Calidad",font.size=14)
    ...
    validacion=as.table(quality_control2(eval(parse(text=svalue(variable)))))
    ...
    addParagraph(info,paste("Para la identificación de datos atípicos se utilizaron",svalue(criterio),"veces el RIC y se estableció un rango de valores permitidos ",svalue(minim),"<",svalue(variable),"<",svalue(maxim),". Nota: Los datos identificados en esta sección son reemplazados por NA's. Los identificados como datos atípicos y los saltos consecutivos, en el caso de la variable temperatura, aquellas con variación mayor a " , svalue(criterio1)," de un día para otro se dejan a criterio del usuario. La última columna de las siguientes tablas indican el % total de datos faltantes que serán llenados en la sección Datos faltantes. Se recomienda que este % no supere el 20% en cada una de las estaciones."))
    endParagraph(info)
    addNewLine(info)
    
    addParagraph(info, paste("Tabla 2.1  Control de calidad para la variable",svalue(variable)))
  ...
  lytgb[8,1]=gbutton("Coordenadas",container=lytgb, expand=F,
                     handler =function(h,...) {load_coor()})
  
  lytgb[9,1]=(glabel=(""))
  
  lytgb[10,1]=glabel("-¿Desea realizar el llenado de datos?")
  lytgb[11,1]=op=(gdroplist(c("Si","No"),handler=function(...) opc() ))
  ...
  lytg.1[3,2]=gbutton("Gráficos Boxplot",container=lytg.1,
                      handler = function(h,...){graf_box() } )
  
  
  lytg.1[4,1]=gbutton("Gráficos de dispersión",container=lytg.1,
                      handler = function(h,...){graf_disp() } )

  lytg.1[4,2]=gbutton("Climogramas",container=lytg.1,
                      handler = function(h,...){graf_clim() } )
  ...
  lytg[7,2]=gbutton("Validar",container=lytg,handler=function(h,...){print(quality_control(eval(parse(text=svalue(variable)))))})
 # lytg[7,3]=gbutton("Gráficos",container=lytg,handler=function(h,...){grafcontrol(eval(parse(text=svalue(variable))))})
  lytg[7,1]=(qc_na=gcheckbox("¿Desea reemplazar los datos por NA?",cont=lytg))
  ...
  lytgg.3[1,2]=(nom_arch1 <-gedit("control_de_calidad",container=lytgg.3,coerce.with=as.character)) #Genera un label editable para el nombre del archivo del informe
  ...
  lyt3[2,1]=gbutton("Completar datos",container=lyt3,handler = function(h,...){datos_falt()},expand=T)
  lyt3[3,1:4]=gtext("Nota: Es indispensable tener los datos de temperatura mínima, máxima y precipitación",container=lyt3,coerce.with=as.character)
  ...
  lyt3[2,1]=gbutton("Completar datos",container=lyt3,handler = function(h,...){datos_falt_chirps()},expand=T)
  lyt3[3,1:4]=gtext("Nota: Esta opción únicamente se aplica para los datos de precipitación mensuales",container=lyt3,coerce.with=as.character)
  ...
  # 
  # lyt6=glayout(homogeneous =T,cont=nb,spacing=2,label="9. Pronósticos Determinísticos (off)",expand=T)
  # 
  ...
  # 
  # lyt7=glayout(homogeneous =F,cont=nb,spacing=2,label="10. Análisis de correlaciones (off) ",expand=T)
  # 
